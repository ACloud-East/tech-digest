...
const AIGenerator = {
    ...
    buildWeiboPrompt(form) {
        ...
        let p = '';
        p += `请写一篇发布到微博的科技爆料/资讯文案，主题：「${topic}」。\n`;
        p += `写作要求（微博体特征：短、快、有梗、能聊）：\n`;
        p += `① 语气像数码博主爆料/科技资讯：接地气、有网感、口语化，但【严禁】小红书种草腔——禁用「姐妹们/宝子们/种草/谁懂啊/太香了/家人们」这类词，保持「爆料/传闻/资讯」的调性；\n`;
        p += `② 多用微博特有写法：「微博透露」「博主表示」「据网友爆料」「媒体结合线索猜测」「评论区有用户问…博主回复道…」；传闻类信息用「据传/疑似/预计/或」等词保留不确定语气，不要写成定论；\n`;
        p += `③ 短文字、快节奏：多用短句、短段，一段一行、频繁换行，像刷微博一样有呼吸感，别写成长段落；前两句就抛爆点/悬念/反差抓人，别铺垫太长；可带 1-2 个 emoji 点缀，不用 ## 小标题；\n`;
        p += `④ 结尾用【强互动钩子】引发评论：抛一个具体能聊的问题（如「这个价格你们觉得香不香？」「这波操作你打几分？」），也可模拟一条「评论区网友：…博主回复：…」增加真实感；并带上 1-2 个话题标签（#vivoS60# #数码圈#）；\n`;
        p += `⑤ 字数控制在约 ${wordCount} 字以内，事实来自下方素材，不编造素材外参数；素材少时写一条 100-150 字短爆料即可，别硬凑字数。\n`;
        if (form.content && form.content.trim().length > 30) {
    ...
    buildPrompt(form) {
        ...
        if (form.plain) {
            if (isEnglish) {
                prompt += `\nOutput format: Markdown. The first line is the title (# Title). Then write a continuous essay-style body WITHOUT ## subheadings and WITHOUT bullet lists — but you MUST develop the material as fully as a structured article (an essay is NOT a shorter summary). Follow this roadmap naturally without labeling it as headings: lead/opening → core specs & parameters explained one by one → use & creative scenarios → differences vs predecessors/competitors → closing. Each part must be substantial. HARD FLOOR: the body must be at least ${Math.round(targetWords * 0.85)} characters (ideally about ${targetWords}); falling short is a failure. Do not pad with fluff, but do not compress either. CRITICAL: the plain/essay version must NOT contain any citation markers like [1], [2] or [?], and must NOT include a references section.`;
            } else {
                prompt += `\n输出要求：用 Markdown 格式，第一行为标题（# 标题），之后写成一篇连贯的、不分 ## 小标题、不用分点列表的散文式正文。但必须像结构式一样把内容充分展开，不可压缩成摘要。建议脉络（不要写成小标题，自然融入行文）：开篇引入 → 核心规格与参数逐条解析 → 使用与创作场景 → 与前代/竞品的差异 → 收束。每一部分都要写充实。硬性下限：正文不得少于约 ${Math.round(targetWords * 0.85)} 字（理想约 ${targetWords} 字）；未达标视为未完成。不要堆空话，但也不要因为不分小标题就缩短篇幅。特别注意：非结构式正文中严禁出现 [1]、[2]、[?] 等引用编号，也不要写「参考来源」小节。`;
            }
        } else {
            if (isEnglish) {
