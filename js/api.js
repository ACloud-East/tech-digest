...
const API = {
    ...
    // ---- 分块缓存：每个 200 篇的小文件独立存 Cache Storage，已下块的冷启动/重启后续传 ----
    async getChunkCache(i) {
        try {
            if (typeof caches === 'undefined') return null;
            const cs = await caches.open('td-chunks-v1');
            const r = await cs.match('/c/' + i);
            return r ? r.json() : null;
        } catch (_) { return null; }
    },
    async setChunkCache(i, data) {
        try {
            if (typeof caches === 'undefined') return;
            const cs = await caches.open('td-chunks-v1');
            await cs.put('/c/' + i, new Response(JSON.stringify(data), { headers: { 'Content-Type': 'application/json' } }));
        } catch (_) {}
    },
    ...
    techSourceConfig: [
        ...
        // ===== 新增国际科技媒体（相机/影像评测、综合科技、日本 PC/汽车/数码）=====
        { key: 'dpreview', name: 'DPReview', color: '#0b66c2' },
        { key: 'cnet', name: 'CNET', color: '#1a73e8' },
        { key: 'impresswatch', name: 'Impress Watch', color: '#b71c1c' },
    ],
    ...
    async fetchAllTechNews(onProgress) {
        ...
        // 3) 历史语料库（v6：分块续传 + 每片存 Cache Storage，根治冷启动/重启/反复开关）。
        //    v5 的痛点：整文件 9.4MB 在你的网络下单次下载常失败；Web Locks 只对"同会话并发窗口"
        //    有效，对冷启动/关浏览器/重启无能为力。缓存因此从没填满，每次重开都从头失败。
        //    解法：把归档切成 40 个 ~182KB 小文件，逐片下载——小文件单请求几乎必成功，
        ...
        const readChunksArchive = async (need, sizes) => {
            let articles = [];
            let loadedBytes = 0;
            ...
            for (let i = 0; i < 80; i++) {
                ...
                if (a.length) { articles.push(...a); if (sizes && sizes[i]) loadedBytes += sizes[i]; }
            }
            if (articles.length >= (need || 1)) return { articles, updateTime: '', loadedBytes };
            return null;
        };
        const downloadChunks = async () => {
            ...
            const articles = collected.filter(Boolean).flat();
            return { articles, updateTime: manifest.updateTime || '', complete: articles.length >= manifest.total };
        };
        const archiveTask = (async () => {
            // 取分块清单（极小文件），拿到每块真实字节数 sizes，用于缓存命中时让进度条显示 X.XX MB，
            // 而不是把文章数误当成字节数（避免出现 "7000.00 MB" 假值），也避免硬刷新走缓存秒开时
            // 完全不 emit 归档进度、导致 4%→96% 的突跳。
            let chunkSizes = null, chunkTotalBytes = 0;
            try {
                const m = await this.fetchJSON('data/news-chunks/manifest.json?_=' + Date.now(), 15000);
                if (m && Array.isArray(m.chunks) && Array.isArray(m.sizes)) {
                    chunkSizes = m.sizes;
                    chunkTotalBytes = m.sizes.reduce((a, b) => a + b, 0);
                }
            } catch (e) { console.warn('读取分块清单失败:', e.message); }
            ...
            if (whole && whole.articles && whole.articles.length >= 7000) {
                ...
                prog.archStarted = true; prog.archIsParts = false; prog.archIsByte = !!chunkTotalBytes;
                prog.archKnown = true;
                // 真实字节数：清单有总字节就用字节（显示 X.XX MB），否则退回文章数（仍 emit，不走突跳）
                prog.archTotal = chunkTotalBytes || whole.articles.length;
                prog.archLoaded = prog.archTotal; emit();
                return { articles: whole.articles, updateTime: whole.updateTime || '' };
            }
            // ── 分块缓存凑齐 ──
            const cached = await readChunksArchive(7000, chunkSizes);
            if (cached) {
                console.warn(`[archive] 分块缓存已齐 ${cached.articles.length} 篇，秒开`);
                // 关键修复：缓存命中也必须 emit 一次真实字节进度，否则 UI 从 4% 直接跳 96%
                prog.archStarted = true; prog.archIsParts = false; prog.archIsByte = !!chunkTotalBytes;
                prog.archKnown = true;
                prog.archTotal = chunkTotalBytes || cached.articles.length;
                prog.archLoaded = (chunkTotalBytes && typeof cached.loadedBytes === 'number') ? cached.loadedBytes : prog.archTotal;
                emit();
            ...
            try {
                ...
                console.warn('[archive] 分块下载异常:', e.message);
            }
            ...
            if (result && result.articles.length) {
                console.warn(`[archive] 本次到位 ${result.articles.length} 篇，剩余块待续传`);
                return result;
            }
            return null;
        })();
