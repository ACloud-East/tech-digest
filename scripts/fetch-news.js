...
const TITLE_BLOCK_T1 = [
    ...
    '世界杯','奥运','亚运','欧冠','NBA','CBA','足球','篮球','排球','网球','演唱会','票房','综艺','明星','电视剧','小说',
    '反腐','贪腐','受贿','判刑','逮捕','通缉','命案','坠楼',
...
const TITLE_BLOCK_T2 = [
    ...
    // 赛事/竞技类（对混合源/主题源仍拦截，纯科技源如 DoNews 不拦）
    '决赛','半决赛','小组赛','冠军','亚军','季军','金牌','银牌','铜牌','转会','签约','续约','退役','复出','伤病','禁赛','奥运冠军',
    // 抗议/灾害类（纯科技源中"玩家抗议""地震预警"是科技文，混合源才拦截）
    '抗议','诈骗',
    '台风','暴雨','洪水','地震','干旱','高温','寒潮','灾情','救援',
    // 电影类（电影机是设备名，电影在纯科技源不拦）
    '电影',
    // 美食/旅游类（纯科技源中旅游/美食版块不拦，混合源才需拦截）
...
const HARD_ENT_TERMS = [
    ...
    // 赛事/体育
    'NBA','CBA','世界杯','奥运','亚运','欧冠','西甲','意甲','德甲','中超','联赛','欧联','亚冠','UFC','WWE','F1','赛车','车队','积分榜','出线','淘汰赛','半决赛','决赛','MVP','FMVP','助攻王','得分王','射手王','金靴','金球','金手套','最佳阵容','卫冕','联赛','赛季','比赛日','对阵','绝杀','逼平','补时','加时','点球','越位','红牌','黄牌','停赛','受伤','复出',
    // 美妆/护肤/医美/瘦身
...
const googleNewsSources = [
    ...
    // 故用 Google News 的 site:fengniao.com 兜底；社区里杂内容多（旅游/风光/汽车/美食/宠物/娱乐等），
    // 先用 keep 白名单保留影像/光学/数码/科技相关条目，再用 drop 黑名单过滤明显非科技内容。
    { name: '蜂鸟网', site: 'fengniao.com', color: '#ff8c00',
      keep: ['相机','镜头','影像','数码','摄影','手机','评测','科技','器材','微单','单反','CMOS','传感器','光学','无人机','平板','笔记本','显示器','耳机','智能','屏幕','芯片','画质','像素','存储','显卡','主板','路由器','快门','变焦','摄像','样张','样片','拍摄','对焦','防抖','连拍','固件','续航','闪存','内存','电池','充电','蓝牙','Wi-Fi','raw','RAW','视频','vlog','旅拍','人像','风光','夜景','街拍','胶片','镜头','滤镜','三脚架','云台','稳定器','航拍'],
      drop: ['旅游','旅行','游记','自驾游','风景','古镇','苗寨','乡村','高原','西藏','新疆','云南','桂林','黔东南','美食','餐厅','吃货','舌尖上的','探店','宠物','猫','狗','萌宠','美女','明星','娱乐','八卦','时尚','穿搭','汽车','轿车','SUV','奥迪','宝马','奔驰','大众','丰田','岚图','比亚迪','特斯拉','蔚来','小鹏','理想','军事','历史','政治','社会','考古','博物馆','星座','运势','心理','情感','鸡汤','房产','楼市','房价','装修','家居'] },
...
async function fetchGoogleNews(src, existingTitles) {
    try {
        ...
        for (const it of (feed.items || [])) {
            ...
            // drop 黑名单：命中任一非科技/非目标关键词则直接丢弃（比 keep 更严格地兜底）
            if (src.drop && src.drop.some(k => title.includes(k))) continue;
...
// 含站点兜底源(华尔街见闻/虎嗅/品玩/极客公园/快科技) 与 主题源(数码测评/新品发布/三星/索尼/尼康/佳能/科技专访)
const MIXED_SOURCES = ['华尔街见闻', '虎嗅', '品玩', '极客公园', '快科技',
    '数码测评', '新品发布', '三星', '索尼', '尼康', '佳能', '科技专访'];
